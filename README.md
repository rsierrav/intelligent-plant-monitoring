...
- `figures/` - final plot images generated by `main.py` for review/demo.
- `data/raw/` - local Supabase CSV cache generated by the analysis script. This folder is ignored and not committed.
...
This uses the same Supabase environment variables as the dashboard. It prints the latest watering decisions in the terminal and saves these review-ready plots:

- `figures/baseline.png`
- `figures/rolling_average.png`
- `figures/watering_trends.png`
- `figures/time_to_dry.png`
- `figures/rolling_with_events.png`

The script also writes a local Supabase cache to `data/raw/`. That raw CSV cache is ignored because it is generated data; the final plot images in `figures/` are the files intended for capstone review.
